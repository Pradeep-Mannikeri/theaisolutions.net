  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className="w-full px-4 py-3 rounded-xl bg-white/90 border-0 focus:ring-2 focus:ring-[#330157] text-[#330157] placeholder-[#66318B]/60 flex items-center justify-between"
      >
      ...
      {isOpen && (
        <div className="absolute z-50 w-full mt-1 bg-white rounded-xl shadow-lg max-h-60 overflow-auto">
          <input
            type="text"
            placeholder="Search country or code..."
            className="w-full p-2 border-b border-gray-200 focus:outline-none focus:ring-2 focus:ring-[#330157] rounded-t-xl"
            value={searchTerm}
            ...
            {filteredCountries.length > 0 ? (
              filteredCountries.map((country) => (
                <button
                  key={country.code}
                  type="button"
                  className={`w-full px-4 py-2 text-left hover:bg-[#F7F4FF] ${
                    country.code === value ? 'bg-[#F7F4FF] text-[#330157]' : 'text-gray-700'
                  }`}
                  onClick={() => {
